...
cp -a build/libdmdutil.{so,so.*} ${PROJECT_SOURCE_ROOT}/third-party/runtime-libs/linux/aarch64/
cd ..
...
cp -a SDL/build/libSDL3.{so,so.*} ${PROJECT_SOURCE_ROOT}/third-party/runtime-libs/linux/aarch64/
